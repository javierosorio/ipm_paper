...
model = transformers.BertForSequenceClassification.from_pretrained(
    pretrain_model, num_labels=5, output_attentions=False, output_hidden_states=False
)
...
def test(device, model, test_loader, annotation_round):
    ...
    try:
        with open(os.path.join(log_folder, result_file), 'a') as f:
...
num_annotation_rounds = 150  # Updated from 200 to 110
num_epochs = 10
