...
def core_set(device, model, x_train_labeled, x_train_unlabeled, tokenizer, batch_size=16, num_samples_to_select=10):
    cls_vector_labeled = []
...
def monte_carlo(device, model, x_train_labeled, x_train_unlabeled, y_train_labeled):
    cls_vector = []
    p_model, optimizer  = get_model(768, 5)
...
def bald_sampling(device, model, unlabeled_tensor, tokenizer=None, num_samples_to_select=10, mc_iterations=10, batch_size=64, pad_token_id=0):
    """
...
for annotation_round in range(num_annotation_rounds):
    ...
    all_items = [item for sublist in lists_of_integers for item in sublist]
    
    # Count the occurrences of each item
    counts = Counter(all_items)
    # Get the 10 most common items
    most_common = counts.most_common(10)
    # Extract only the items (not the counts) and return them
    for item, count in most_common:
        print(f"Item: {item}, Occurrences: {count}")

    # Initialize a dictionary to store position-based scores
    position_scores = defaultdict(int)

    # Assign scores based on the index of each item in each list
    for lst in lists_of_integers:
        for idx, item in enumerate(lst):
            position_scores[item] += len(lst) - idx  # Higher score for items appearing earlier

    # Combine frequency and position scores
    scored_items = {item: (counts[item], position_scores[item]) for item in counts}

    # Sort items based on frequency first, then position scores
    sorted_items = sorted(scored_items.items(), key=lambda x: (-x[1][0], -x[1][1]))

    # Get the top 10 most common items with position priority
    selected_indices = [item for item, _ in sorted_items[:num_samples_to_select]]
